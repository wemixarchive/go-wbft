...
func TestCheckSignature(t *testing.T) {
	...
	a = getInvalidAddress()
	err = b.CheckSignature(data, a, sig)
	if err != qbftcommon.ErrInvalidSignature {
		t.Errorf("error mismatch: have %v, want %v", err, qbftcommon.ErrInvalidSignature)
	}
}
...
func TestCheckValidatorSignature(t *testing.T) {
	...
	for i, k := range keys {
		...
		addr, err := qbft.CheckValidatorSignature(vset, data, sig)
		if err != nil {
			t.Errorf("error mismatch: have %v, want nil", err)
		}
	...
	addr, err := qbft.CheckValidatorSignature(vset, data, sig)
	if err != qbft.ErrUnauthorizedAddress {
		t.Errorf("error mismatch: have %v, want %v", err, qbft.ErrUnauthorizedAddress)
	}
...
func newTestValidatorSet(n int) (qbft.ValidatorSet, []*ecdsa.PrivateKey) {
	// generate validators
	keys := make(Keys, n)
