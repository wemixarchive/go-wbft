...
var (
	...
	errDecodeFailed = errors.New("fail to decode qbft message")
...
func (sb *Backend) HandleMsg(addr common.Address, msg p2p.Msg) (bool, error) {
	...
	if _, ok := qbftmessage.MessageCodes()[msg.Code]; ok || msg.Code == istanbulMsg {
		if !sb.coreStarted {
			return true, qbft.ErrStoppedEngine
		}
		...
		go sb.istanbulEventMux.Post(qbft.MessageEvent{
			Code:    msg.Code,
			Payload: data,
		})
		return true, nil
	...
	if msg.Code == NewBlockMsg && sb.core != nil && sb.core.IsProposer() { // eth.NewBlockMsg: import cycle
		...
		if reader, ok := msg.Payload.(*bytes.Reader); ok {
			...
			if newRequestedBlock.Header().Difficulty == types.QBFTDefaultDifficulty && sb.core.IsCurrentProposal(newRequestedBlock.Hash()) {
...
func (sb *Backend) NewChainHead() error {
	...
	go sb.istanbulEventMux.Post(qbft.FinalCommittedEvent{})
	return nil
}
