...
func (c *Core) newRoundChangeTimer() {
	...
	baseTimeout := time.Duration(c.config.GetConfig(c.current.Sequence(), c.backend.ChainConfig()).RequestTimeout) * time.Millisecond
	round := c.current.Round().Uint64()
	maxRequestTimeout := time.Duration(c.config.GetConfig(c.current.Sequence(), c.backend.ChainConfig()).MaxRequestTimeoutSeconds) * time.Second
