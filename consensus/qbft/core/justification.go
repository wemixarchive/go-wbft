...
//
// This file is derived from quorum/consensus/istanbul/qbft/core/justification.go (2024.07.25).
...
func isJustified(
	...
	roundChangeMessages []*qbftmessage.SignedRoundChangePayload,
...
// prepared block.
func hasQuorumOfRoundChangeMessagesForNil(roundChangeMessages []*qbftmessage.SignedRoundChangePayload, quorumSize int) error {
	nilCount := 0
	for _, m := range roundChangeMessages {
		log.Trace("QBFT: hasQuorumOfRoundChangeMessagesForNil", "rc", m)
		if (m.PreparedRound == nil || m.PreparedRound.Cmp(common.Big0) == 0) && m.PreparedDigest == (common.Hash{}) {
			nilCount++
...
func hasQuorumOfRoundChangeMessagesForPreparedRoundAndBlock(roundChangeMessages []*qbftmessage.SignedRoundChangePayload, preparedRound *big.Int, preparedBlock qbft.Proposal, quorumSize int) error {
...
// preparedRound and preparedBlockDigest of a ROUND-CHANGE qbftmessage.
func hasMatchingRoundChangeAndPrepares(
	roundChange *qbftmessage.RoundChange, prepareMessages []*qbftmessage.Prepare, quorumSize int) error {
