...
//
// This file is derived from quorum/consensus/istanbul/qbft/core/justification_test.go (2024.07.25).
...
func testParameterizedCase(
	...
	roundChangeMessages := make([]*qbftmessage.SignedRoundChangePayload, 0)
	for index, validator := range validatorSet.List() {
		var m *qbftmessage.SignedRoundChangePayload
		if index < rcForNil {
	...
	// PREPARE messages
	prepareMessages := make([]*qbftmessage.Prepare, 0)
	for index, validator := range validatorSet.List() {
		var m *qbftmessage.Prepare
...
func createRoundChangeMessage(from common.Address, round int64, preparedRound int64, preparedBlock qbft.Proposal) *qbftmessage.SignedRoundChangePayload {
	m := qbftmessage.NewRoundChange(big.NewInt(1), big.NewInt(1), big.NewInt(preparedRound), preparedBlock)
	m.SetSource(from)
