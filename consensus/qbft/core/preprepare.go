...
func (c *Core) sendPreprepareMsg(request *Request) {
	...
	if c.current.Sequence().Cmp(request.Proposal.Number()) == 0 && c.IsProposer() {
		...
		preprepare := qbftmessage.NewPreprepare(curView.Sequence, curView.Round, request.Proposal)
		...
		if request.RCMessages != nil {
			preprepare.JustificationRoundChanges = make([]*qbftmessage.SignedRoundChangePayload, 0)
			for _, m := range request.RCMessages.Values() {
				preprepare.JustificationRoundChanges = append(preprepare.JustificationRoundChanges, &m.(*qbftmessage.RoundChange).SignedRoundChangePayload)
				withMsg(logger, preprepare).Trace("QBFT: add ROUND-CHANGE justification", "rc", m.(*qbftmessage.RoundChange).SignedRoundChangePayload)
			}
	...
	if duration, err := c.backend.Verify(preprepare.Proposal); err != nil {
		...
		if err == consensus.ErrFutureBlock {
			...
			c.futurePreprepareTimer = time.AfterFunc(duration, func() {
				...
				if validator == nil {
					logger.Warn("QBFT: failed to find validator for PRE-PREPARE message", "source", preprepare.Source())
				}
