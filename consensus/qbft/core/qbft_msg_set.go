...
//
// This file is derived from quorum/consensus/istanbul/qbft/core/qbft_msg_set.go (2024.07.25).
...
func newQBFTMsgSet(valSet qbft.ValidatorSet) *qbftMsgSet {
	return &qbftMsgSet{
		...
		messages:   make(map[common.Address]qbftmessage.QBFTMessage),
		valSet:     valSet,
	}
}
...
type qbftMsgSet struct {
	...
	messages   map[common.Address]qbftmessage.QBFTMessage
}
...
func (ms *qbftMsgSet) View() *qbft.View {
	return ms.view
}

func (ms *qbftMsgSet) Add(msg qbftmessage.QBFTMessage) error {
	ms.messagesMu.Lock()
...
func (ms *qbftMsgSet) Values() (result []qbftmessage.QBFTMessage) {
	ms.messagesMu.Lock()
	defer ms.messagesMu.Unlock()
...
func (ms *qbftMsgSet) Get(addr common.Address) qbftmessage.QBFTMessage {
	ms.messagesMu.Lock()
	defer ms.messagesMu.Unlock()
