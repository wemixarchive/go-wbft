...
//
// This file is derived from quorum/consensus/istanbul/qbft/core/roundchange.go (2024.07.25).
...
func (c *Core) broadcastRoundChange(round *big.Int) {
	...
	roundChange := qbftmessage.NewRoundChange(c.current.Sequence(), round, c.current.preparedRound, c.current.preparedBlock)
...
func (c *Core) handleRoundChangeMsg(roundChange *qbftmessage.RoundChange) error {
	...
	if view.Round.Cmp(currentRound) >= 0 {
		var prepareMessages []*qbftmessage.Prepare = nil
		var pr *big.Int = nil
	...
	if float64(num) > c.valSet.F() && float64(num) <= c.valSet.F()+1 {
		...
		rcSignedPayloads := make([]*qbftmessage.SignedRoundChangePayload, 0)
		for _, m := range roundChangeMessages.Values() {
			rcMsg := m.(*qbftmessage.RoundChange)
...
func newRoundChangeSet(valSet qbft.ValidatorSet) *roundChangeSet {
	return &roundChangeSet{
		validatorSet:         valSet,
		roundChanges:         make(map[uint64]*qbftMsgSet),
		prepareMessages:      make(map[uint64][]*qbftmessage.Prepare),
		highestPreparedRound: make(map[uint64]*big.Int),
		highestPreparedBlock: make(map[uint64]qbft.Proposal),
		mu:                   new(sync.Mutex),
	}
}
...
type roundChangeSet struct {
	...
	prepareMessages      map[uint64][]*qbftmessage.Prepare
	highestPreparedRound map[uint64]*big.Int
	highestPreparedBlock map[uint64]qbft.Proposal
	mu                   *sync.Mutex
}
...
func (rcs *roundChangeSet) NewRound(r *big.Int) {
	...
	if rcs.prepareMessages[round] == nil {
		rcs.prepareMessages[round] = make([]*qbftmessage.Prepare, 0)
	}
}

// Add adds the round and message into round change set
func (rcs *roundChangeSet) Add(r *big.Int, msg qbftmessage.QBFTMessage, preparedRound *big.Int, preparedBlock qbft.Proposal, prepareMessages []*qbftmessage.Prepare, quorumSize int) error {
	rcs.mu.Lock()
	defer rcs.mu.Unlock()
	...
	if preparedRound != nil && (rcs.highestPreparedRound[round] == nil || preparedRound.Cmp(rcs.highestPreparedRound[round]) > 0) {
		roundChange := msg.(*qbftmessage.RoundChange)
