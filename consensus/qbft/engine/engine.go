...
func (e *Engine) verifyPreparedSeals(chain consensus.ChainHeaderReader, header *types.Header, parents []*types.Header, validators qbft.ValidatorSet, prevValidators qbft.ValidatorSet) error {
	number := header.Number.Uint64()
	...
	if len(prevPreparedSeal) == 0 {
		...
		prevPreparers, err = e.PrepareSigners(prevHeader)
		if err != nil {
			return err
		}
...
func (e *Engine) verifyCommittedSeals(chain consensus.ChainHeaderReader, header *types.Header, parents []*types.Header, validators qbft.ValidatorSet, prevValidators qbft.ValidatorSet) error {
	...
	if len(prevCommittedSeal) == 0 {
		...
		prevHeader := chain.GetHeaderByNumber(number - 1)
		prevCommitters, err = e.CommitSigners(prevHeader)
		if err != nil {
			return err
		}
...
func (e *Engine) PrepareSigners(header *types.Header) ([]common.Address, error) {
	...
	// 1. Get prepared seals from current header
	for _, seal := range preparedSeal {
