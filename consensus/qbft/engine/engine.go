...
func (e *Engine) verifyCascadingFields(chain consensus.ChainHeaderReader, header *types.Header, validators qbft.ValidatorSet, prevValidators qbft.ValidatorSet, parents []*types.Header, checkSeal bool) error {
	...
	latestEpoch, latestEpochInfo, err := e.GetEpochInfo(chain, header, parents)
	if err != nil {
		return err
	}
	...
	if firstWbftBlockNum.Cmp(header.Number) != 0 && number != 1 {
		...
		if latestEpoch.Cmp(parent.Number) == 0 && firstWbftBlockNum.Cmp(parent.Number) != 0 {
			...
			_, epochInfo, err := e.GetEpochInfo(chain, parent, prevParents)
			if err != nil {
				return err
			}
		...
		if err := e.verifyPrevSeals(header, parent, prevValidators, prevEpochInfo); err != nil {
			return err
		}
	}
...
func (e *Engine) verifyPrevSeals(header *types.Header, parent *types.Header, prevValidators qbft.ValidatorSet, epochInfo *types.EpochInfo) error {
	number := header.Number.Uint64()
