...
//
// This file is derived from quorum/consensus/istanbul/backend/backend_test.go (2024.07.25).
...
func TestCheckSignature(t *testing.T) {
	...
	a = getInvalidAddress()
	err = b.CheckSignature(data, a, sig)
	if err != wbftcommon.ErrInvalidSignature {
		t.Errorf("error mismatch: have %v, want %v", err, wbftcommon.ErrInvalidSignature)
	}
}
...
func TestCheckValidatorSignature(t *testing.T) {
	...
	for i, k := range keys {
		...
		addr, err := wbft.CheckValidatorSignature(vset, data, sig)
		if err != nil {
			t.Errorf("error mismatch: have %v, want nil", err)
		}
	...
	addr, err := wbft.CheckValidatorSignature(vset, data, sig)
	if err != wbft.ErrUnauthorizedAddress {
		t.Errorf("error mismatch: have %v, want %v", err, wbft.ErrUnauthorizedAddress)
	}
...
func TestCommit(t *testing.T) {
	...
	testCases := []struct {
		expectedErr              error
		expectedPrepareSignature []wbft.SealData
		expectedCommitSignature  []wbft.SealData
		...
		},
		{
			...
			wbftcommon.ErrInvalidCommittedSeals,
			[]wbft.SealData{{Seal: testSignature, Sealer: 0}},
			nil,
			func() *types.Block {
				chain, engine, _ := newBlockChain(1)
...
// TestWBFTTransitionDeadlock test whether a deadlock occurs when testWBFTBlock is set to 1
// This was fixed as part of commit 2a8310663ecafc0233758ca7883676bf568e926e
func TestWBFTTransitionDeadlock(t *testing.T) {
...
func newTestValidatorSet(n int) (wbft.ValidatorSet, []*ecdsa.PrivateKey) {
	// generate validators
	keys := make(Keys, n)
	...
	vset := validator.NewSet(addrs, blsPubKeys, wbft.NewRoundRobinProposerPolicy())
	return vset, keys
}
