...
// Its purpose is to test pure wbft/core functions.
// not recommended for testing logic that includes wbft/backend function
func makeCoreForTest(priorRound, currentRound, currentSequence *big.Int, lastProposal *types.Block) *Core {
	...
	valSet := validator.NewSet(validators, blsPubKeys, wbft.NewRoundRobinProposerPolicy())
	core := &Core{
		config:             nil,
...
func TestProcessExtraSeal(t *testing.T) {
	...
	testExtraSealMessages := []messages.WBFTMessage{
...
func createPreprepareMsg(sequence, round *big.Int, proposal wbft.Proposal) *messages.Preprepare {
