...
//
// This file is derived from quorum/consensus/istanbul/wbft/core/handler.go (2024.07.25).
...
func (c *Core) Stop() error {
	c.logger.Info("WBFT: stopping...")
	c.stopTimer()
	c.unsubscribeEvents()

	// Make sure the handler goroutine exits
	c.handlerWg.Wait()
	c.logger.Info("WBFT: stopped")
	return nil
}
	...
	for {
		...
		case event, ok := <-c.events.Chan():
			...
			case backlogEvent:
				...
				if err != nil {
					c.logger.Error("WBFT: can not encode backlog message", "err", err)
					continue
				}
...
func (c *Core) handleEncodedMsg(code uint64, data []byte) error {
	...
	if _, ok := wbfmessage.MessageCodes()[code]; !ok {
		logger.Error("WBFT: invalid message event code")
		return fmt.Errorf("invalid message event code %v", code)
	}

	// Decode data into a WBFTMessage
	m, err := wbfmessage.Decode(code, data)
	if err != nil {
		logger.Error("WBFT: invalid message", "err", err)
		return err
	}
...
func (c *Core) deliverMessage(m wbfmessage.WBFTMessage) error {
	var err error

	switch m.Code() {
	case wbfmessage.PreprepareCode:
		err = c.handlePreprepareMsg(m.(*wbfmessage.Preprepare))
	case wbfmessage.PrepareCode:
		err = c.handlePrepareMsg(m.(*wbfmessage.Prepare))
	case wbfmessage.CommitCode:
		err = c.handleCommitMsg(m.(*wbfmessage.Commit))
	case wbfmessage.RoundChangeCode:
		err = c.handleRoundChangeMsg(m.(*wbfmessage.RoundChange))
	default:
		c.logger.Error("WBFT: invalid messages code", "code", m.Code())
...
func (c *Core) handleTimeoutMsg() {
	...
	logger.Warn("WBFT: TIMER CHANGING ROUND", "pr", c.current.preparedRound)
	c.startNewRound(nextRound)
	logger.Warn("WBFT: TIMER CHANGED ROUND", "pr", c.current.preparedRound)
...
func (c *Core) verifySignatures(m wbfmessage.WBFTMessage) error {
	logger := c.currentLogger(true, m)

	// Anonymous function to verify the signature of a single message or payload
	verify := func(m wbfmessage.WBFTMessage) error {
		payload, err := m.EncodePayloadForSigning()
		if err != nil {
			logger.Error("WBFT: invalid message payload", "err", err)
			return err
		}
		source, err := c.validateFn(payload, m.Signature(), m.View())
		if err != nil {
			logger.Error("WBFT: invalid message signature", "err", err)
...
func (c *Core) currentLogger(state bool, msg wbfmessage.WBFTMessage) log.Logger {
	logCtx := []interface{}{}
	if c.current != nil {
...
func withMsg(logger log.Logger, msg wbfmessage.WBFTMessage) log.Logger {
	return logger.New(
		"msg.code", msg.Code(),
		"msg.source", msg.Source().String(),
		"msg.round", msg.View().Round.Uint64(),
