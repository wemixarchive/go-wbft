...
//
// This file is derived from quorum/consensus/istanbul/wbft/core/justification_test.go (2024.07.25).
...
func testParameterizedCase(
	...
	roundChangeMessages := make([]*wbfmessage.SignedRoundChangePayload, 0)
	for index, validator := range validatorSet.List() {
		var m *wbfmessage.SignedRoundChangePayload
		if index < rcForNil {
	...
	// PREPARE messages
	prepareMessages := make([]*wbfmessage.Prepare, 0)
	for index, validator := range validatorSet.List() {
		var m *wbfmessage.Prepare
...
func createPrepareMessage(from common.Address, round int64, preparedBlock wbft.Proposal) *wbfmessage.Prepare {
	return wbfmessage.NewPrepareWithSigAndSource(big.NewInt(1), big.NewInt(round), preparedBlock.Hash(), nil, from, nil)
}

func generateValidators(n int) []wbft.Validator {
	vals := make([]wbft.Validator, 0)
	for i := 0; i < n; i++ {
