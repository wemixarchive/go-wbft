...
func (c *Core) sendByzantinePreprepareMsg(hook btypes.ConsensusHook, request *Request, attacks map[btypes.AttackType]*btypes.ExecutableAttack) bool {
	...
	// Byzantine - Check for role spoof attack first
	if at := attacks[btypes.AttackTypeRoleSpoofed]; at != nil && at.RoleSpoofParams != nil {
		// Check if fields specify non-proposer attack behavior
		shouldExecute := false

		// Check fields for specific attack behavior
		...
		if shouldExecute {
			log.Info("[BYZ] attack",
				...
				"has_rc_messages", request.RCMessages != nil)
