...
//
// This file is derived from quorum/consensus/istanbul/wbft/core/types.go
...
type Backend interface {
	...
	Validators(proposal wbft.Proposal) wbft.ValidatorSet

	// EventMux returns the event mux in backend
	EventMux() *event.TypeMux

	// Broadcast sends a message to all validators (include self)
	Broadcast(valSet wbft.ValidatorSet, code uint64, payload []byte) error

	// Gossip sends a message to all validators (exclude self)
	Gossip(valSet wbft.ValidatorSet, code uint64, payload []byte) error

	// Commit delivers an approved proposal to backend.
	// The delivered proposal will be put into blockchain.
	Commit(proposal wbft.Proposal, preparedSeals, committedSeals []wbft.SealData, round *big.Int) error

	// Verify verifies the proposal. If a consensus.ErrFutureBlock error is returned,
	// the time difference of the proposal and current time is also returned.
	Verify(wbft.Proposal) (time.Duration, error)
