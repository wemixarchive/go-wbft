...
//
// This file is derived from quorum/consensus/istanbul/wbft/types/roundchange.go (2024.07.25).
...
func NewRoundChange(sequence *big.Int, round *big.Int, preparedRound *big.Int, preparedBlock wbft.Proposal) *RoundChange {
...
func (p *SignedRoundChangePayload) DecodeRLP(stream *rlp.Stream) error {
	...
	if _, err := stream.List(); err != nil {
		log.Error("WBFT: Error List() Signed Payload", "struct", "SignedRoundChangePayload", "err", err)
		return err
	}
	...
	if err != nil {
		log.Error("WBFT: Error Raw()", "err", err)
		return err
	}
	...
	if _, err = payloadStream.List(); err != nil {
		log.Error("WBFT: Error List() Payload", "err", err)
		return err
	}

	if err = payloadStream.Decode(&p.Sequence); err != nil {
		log.Error("WBFT: Error Decode(&m.Sequence)", "err", err)
		return err
	}
	if err = payloadStream.Decode(&p.Round); err != nil {
		log.Error("WBFT: Error Decode(&m.Round)", "err", err)
		return err
	}
	...
	if size, err = payloadStream.List(); err != nil {
		log.Error("WBFT: Error List() Prepared", "err", err)
		return err
	}
	if size > 0 {
		if err = payloadStream.Decode(&p.PreparedRound); err != nil {
			log.Error("WBFT: Error Decode(&m.PreparedRound)", "err", err)
			return err
		}
		if err = payloadStream.Decode(&p.PreparedDigest); err != nil {
			log.Error("WBFT: Error Decode(&p.PreparedDigest)", "err", err)
			return err
		}
	}
	// End Prepared
...
func (m *RoundChange) DecodeRLP(stream *rlp.Stream) error {
	...
	// Signed Payload
	if _, err = stream.List(); err != nil {
		log.Error("WBFT: Error List() Signed Payload", "err", err)
		return err
	}
	...
	if size > 0 {
		...
		if err = payloadStream.Decode(&m.PreparedDigest); err != nil {
			log.Error("WBFT: Error Decode(&m.PreparedDigest)", "err", err)
			return err
		}
	}
	// End Prepared
	...
	if _, size, err = stream.Kind(); err != nil {
		log.Error("WBFT: Error Kind()", "err", err)
		return err
	}
	if size == 0 {
	...
	if size == 0 {
		...
		if err = stream.Decode(&m.Justification); err != nil {
			log.Error("WBFT: Error Decode(&m.Justification)", "err", err)
			return err
		}
	}
