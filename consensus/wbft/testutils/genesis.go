...
func GenesisWithSeals(validators []common.Address, blsPublicKeys [][]byte) *core.Genesis {
	...
	genesis.Config = params.TestWBFTChainConfig
...
func appendValidatorsAndPrevSeals(genesis *core.Genesis, validators []common.Address, blsPublicKeys [][]byte) {
	setWBFTExtra(genesis, validators, blsPublicKeys, true)
}

func setWBFTExtra(genesis *core.Genesis, validators []common.Address, blsPublicKeys [][]byte, withPrev bool) {
	...
	if withPrev {
		ist.PrevRound = 0
		ist.PrevPreparedSeal = &types.WBFTAggregatedSeal{Signature: []byte{}, Sealers: types.SealerSet{}}
		ist.PrevCommittedSeal = &types.WBFTAggregatedSeal{Signature: []byte{}, Sealers: types.SealerSet{}}
	}

	istPayload, err := rlp.EncodeToBytes(&ist)
	if err != nil {
		panic("failed to encode wbft extra")
	}
