...
func (wpoa *WemixPoA) VerifyDynamicGasHeader(config *params.ChainConfig, parent, header *types.Header, gasTargetPercentage uint64) error {
	// Verify that the gas limit remains within allowed bounds
	parentGasLimit := parent.GasLimit
	if !config.IsLondon(parent.Number) {
		parentGasLimit = parent.GasLimit * gasTargetPercentage / 100
	}
	...
	expectedBaseFee := wpoa.CalcBaseFee(config, parent)
...
func (wpoa *WemixPoA) CalcBaseFee(config *params.ChainConfig, parent *types.Header) *big.Int {
	...
	_, maxBaseFeeGov, _, baseFeeMaxChangeRate, gasTargetPercentage, err := wpoa.getBlockBuildParameters(parent.Number)
