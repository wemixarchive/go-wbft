...
type SetCodeTx struct {
	...
	Gas        uint64
	To         common.Address // SetCodeTx cannot create contracts, so To is never nil
	Value      *uint256.Int
	Data       []byte
...
func (tx *SetCodeTx) effectiveGasPrice(dst *big.Int, baseFee *big.Int) *big.Int {
	...
	txTipCap := tx.GasTipCap.ToBig()
	if tip.Cmp(txTipCap) > 0 {
		tip.Set(txTipCap)
	}
...
func (tx *SetCodeTx) feePayer() *common.Address { return nil }
