...
func (evm *EVM) precompile(addr common.Address) (PrecompiledContract, bool) {
	...
	switch {
	case evm.chainRules.IsCroissant:
		precompiles = PrecompiledContractsCroissant
...
func NewEVM(blockCtx BlockContext, txCtx TxContext, statedb StateDB, chainConfig *params.ChainConfig, config Config) *EVM {
	...
	evm := &EVM{
		...
		chainRules:  chainConfig.Rules(blockCtx.BlockNumber, (blockCtx.Difficulty == nil || blockCtx.Difficulty.Cmp(common.Big0) == 0) && blockCtx.Random != nil, blockCtx.Time),
	}
...
func (evm *EVM) Call(caller ContractRef, addr common.Address, input []byte, gas uint64, value *uint256.Int) (ret []byte, leftOverGas uint64, err error) {
	...
	if !evm.StateDB.Exist(addr) {
		if !isPrecompile && evm.chainRules.IsEIP158 && value.IsZero() {
			...
			if debug {
				if evm.depth == 0 {
					evm.Config.Tracer.CaptureStart(caller.Address(), addr, false, input, gas, value.ToBig())
...
func (evm *EVM) create(caller ContractRef, codeAndHash *codeAndHash, gas uint64, value *uint256.Int, address common.Address, typ OpCode) ([]byte, common.Address, uint64, error) {
	...
	if evm.Config.Tracer != nil {
		if evm.depth == 0 {
			evm.Config.Tracer.CaptureStart(caller.Address(), address, true, codeAndHash.code, gas, value.ToBig())
		} else {
...
// In Croissant (EIP-7702, from Ethereum Prague), it can also resolve code
// pointed to by a delegation designator.
func (evm *EVM) resolveCode(addr common.Address) []byte {
	code := evm.StateDB.GetCode(addr)
	if !evm.chainRules.IsCroissant {
		return code
	}
...
// In Croissant (EIP-7702, from Ethereum Prague), it can also resolve code hash
// of the account pointed to by a delegation designator. Although this is not
// accessible in the EVM it is used internally to associate jumpdest analysis to code.
