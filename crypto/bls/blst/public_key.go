...
// Original source: https://github.com/OffchainLabs/prysm/blob/develop/crypto/bls/blst/public_key_test.go
...
var pubkeyCache *lru.Cache[[48]byte, common.PublicKey]
...
func publicKeyFromBytes(pubKey []byte, cacheCopy bool) (common.PublicKey, error) {
	if len(pubKey) != common.BLS_PUBLIC_KEY_LENGTH {
		return nil, fmt.Errorf("public key must be %d bytes", common.BLS_PUBLIC_KEY_LENGTH)
	}

	// from cache
	newKey := (*[common.BLS_PUBLIC_KEY_LENGTH]byte)(pubKey)
