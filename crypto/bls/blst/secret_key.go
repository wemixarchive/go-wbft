...
// Original source: https://github.com/OffchainLabs/prysm/blob/develop/crypto/bls/blst/secret_key.go
...
func GenerateKey(ikm []byte) (common.SecretKey, error) {
	if ikm == nil {
		return nil, common.ErrInvalidIKM
	}
...
func SecretKeyFromBytes(privKey []byte) (common.SecretKey, error) {
	if len(privKey) != common.BLS_SECRET_KEY_LENGTH {
		return nil, fmt.Errorf("secret key must be %d bytes", common.BLS_SECRET_KEY_LENGTH)
	}
