...
// Original source: https://github.com/OffchainLabs/prysm/blob/develop/crypto/bls/blst/signature.go
...
func signatureFromBytesNoValidation(sig []byte) (*blstSignature, error) {
	if len(sig) != common.BLS_SIGNATURE_LENGTH {
		return nil, fmt.Errorf("signature must be %d bytes", common.BLS_SIGNATURE_LENGTH)
	}
...
func MultipleSignaturesFromBytes(multiSigs [][]byte) ([]common.Signature, error) {
	...
	for _, s := range multiSigs {
		if len(s) != common.BLS_SIGNATURE_LENGTH {
			return nil, fmt.Errorf("signature must be %d bytes", common.BLS_SIGNATURE_LENGTH)
		}
	}
