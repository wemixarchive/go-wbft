...
// Original source: https://github.com/OffchainLabs/prysm/blob/develop/crypto/bls/blst/secret_key_test.go
...
func TestZeroKey(t *testing.T) {
	// Is Zero
	var zKey [32]byte
	require.True(t, blst.IsZero(zKey[:]))

	// Is Not Zero
	_, err := rand.Read(zKey[:])
	require.NoError(t, err)
	require.False(t, blst.IsZero(zKey[:]))
}
