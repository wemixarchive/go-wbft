...
// Original source: https://github.com/OffchainLabs/prysm/blob/develop/crypto/bls/blst/signature_test.go
...
import (
	...
	sblst "github.com/supranational/blst/bindings/go"
)
...
func TestSignVerify(t *testing.T) {
	...
	msg := []byte("hello")
	sig := priv.Sign(msg)
	require.True(t, sig.Verify(pub, msg), "Signature did not verify")
}
...
func TestAggregateVerify_CompressedSignatures(t *testing.T) {
	...
	require.NoError(t, err)
	require.Equal(t, aggSig.Marshal(), aggSig2.Marshal(), "Signature did not match up")
}
...
func TestVerifyCompressed(t *testing.T) {
	...
	msg := []byte("hello")
	sig := priv.Sign(msg)
	require.True(t, sig.Verify(pub, msg), "Non compressed signature did not verify")
	require.True(t, blst.VerifyCompressed(sig.Marshal(), pub.Marshal(), msg), "Compressed signatures and pubkeys did not verify")
}
...
func TestVerifySingleSignature_InvalidSignature(t *testing.T) {
	...
	valid, err := blst.VerifySignature(sigA, msgB, pub)
	require.NoError(t, err)
	require.False(t, valid, "Signature did verify")
}
...
func TestVerifySingleSignature_ValidSignature(t *testing.T) {
	...
	valid, err := blst.VerifySignature(sig, msg, pub)
	require.NoError(t, err)
	require.True(t, valid, "Signature did not verify")
}
...
func TestMultipleSignatureVerification(t *testing.T) {
	...
	require.True(t, verify, "Signature did not verify")
}
...
func TestFastAggregateVerify_ReturnsFalseOnEmptyPubKeyList(t *testing.T) {
	...
	require.False(t, aggSig.FastAggregateVerify(pubkeys, msg), "Expected FastAggregateVerify to return false with empty input ")
}
...
func TestEth2FastAggregateVerify_ReturnsFalseOnEmptyPubKeyList(t *testing.T) {
	...
	aggSig := blst.NewAggregateSignature()
	require.False(t, aggSig.Eth2FastAggregateVerify(pubkeys, msg), "Expected Eth2FastAggregateVerify to return false with empty input ")
}
...
func TestEth2FastAggregateVerify_ReturnsTrueOnG2PointAtInfinity(t *testing.T) {
	...
	aggSig, err := blst.SignatureFromBytes(g2PointAtInfinity)
	require.NoError(t, err)
	require.True(t, aggSig.Eth2FastAggregateVerify(pubkeys, msg))
}
...
func TestSignatureFromBytesNoValidation(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			res, err := blst.SignatureFromBytesNoValidation(test.input)
			if test.err != nil {
				require.NotEqual(t, nil, err, "No error returned")
				require.ErrorContains(t, err, test.err.Error(), "Unexpected error returned")
			} else {
...
func TestMultipleSignatureFromBytes(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			res, err := blst.MultipleSignaturesFromBytes(test.input)
			if test.err != nil {
				require.NotEqual(t, nil, err, "No error returned")
				require.ErrorContains(t, err, test.err.Error(), "Unexpected error returned")
			} else {
				require.NoError(t, err)
				for i, s := range res {
					require.Equal(t, 0, bytes.Compare(s.Marshal(), test.input[i]))
				}
			}
		})
	}
...
func TestCopy(t *testing.T) {
	...
	signatureA := &testSignature{s: new(sblst.P2Affine).Sign(p, []byte("foo"), dst)}
	signatureB := signatureA.Copy()

	require.False(t, signatureA == signatureB)
	require.False(t, signatureA.s == signatureB.s)
	require.Equal(t, signatureA, signatureB)

	signatureA.s.Sign(p, []byte("bar"), dst)
	require.NotEqual(t, signatureA, signatureB)
}
...
func (s *testSignature) Copy() *testSignature {
	sign := *s.s
	return &testSignature{s: &sign}
}
