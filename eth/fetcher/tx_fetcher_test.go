...
func TestTransactionProtocolViolation(t *testing.T) {
	...
	testTransactionFetcherParallel(t, txFetcherTest{
		...
		steps: []interface{}{
			// Initial announcement to get a badTx into the waitlist
			doTxNotify{
				peer:   "A",
			...
			// Inject an interleaved valid transaction to ensure later
			// disconnected traces do not leave partial dangling state
			doTxNotify{
				peer:   "A",
				hashes: []common.Hash{testTxs[0].Hash()},
