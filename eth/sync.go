...
func (cs *chainSyncer) nextSyncOp() *chainSyncOp {
	...
	if cs.handler.chain.Config().MontBlanc == nil && op.td.Cmp(ourTD) <= 0 {
	...
	} else if cs.handler.chain.Config().MontBlanc != nil && op.td.Cmp(new(big.Int).Add(ourTD, big.NewInt(tdAdjustment))) <= 0 {
