...
func versionInfo(info *debug.BuildInfo) string {
	// If the main package is from our repo, prefix version with "gwemix".
	if strings.HasPrefix(info.Path, ourPath) {
		return fmt.Sprintf("gwemix %s", info.Main.Version)
	}
	...
	if mod == nil {
		...
		// version.
		return version + fmt.Sprintf("gwemix %s", params.VersionWithMeta)
	}
