...
contract GovStaking {
    ...
    function executeChangingFee(address _staker) external {
        ...
        require(block.timestamp - changingFeeRequests[_staker].requestTime >= GOV_CONST.changeFeeDelay(),
    ...
    function _updateRewardInfo(address _staker, address _user) private {
        ...
        if (_stakerInfo.totalStaked > 0) {
            ...
            if (_user != address(0)) {
                ...
                _userInfo.pendingReward += _userInfo.stakingAmount * (_stakerInfo.accRewardPerStaking - _userInfo.rewardPerStaking) / GOV_CONST.rewardPrecision();
                _userInfo.pendingFee += _userInfo.stakingAmount * (_stakerInfo.accFeePerStaking - _userInfo.feePerStaking) / GOV_CONST.rewardPrecision();
        ...
        if (changingFeeRequests[_staker].requestTime > 0 && block.timestamp - changingFeeRequests[_staker].requestTime >= GOV_CONST.changeFeeDelay()) {
