...
func TestOperatorContractMultiSig(t *testing.T) {
	...
	t.Run("Register Staker", func(t *testing.T) {
		...
		t.Run("Failure cases", func(t *testing.T) {
			...
			ExpectedRevert(t, g.ExpectedFail(g.SingleOwnerRegisterStaker(owner1, s1, minStaking, feeRate)), "MultiSig: Only Wallet can access.")
	...
	t.Run("Stake", func(t *testing.T) {
		...
		t.Run("Failure cases", func(t *testing.T) {
			...
			ExpectedRevert(t, g.ExpectedFail(g.SingleOwnerStake(owner1, minStaking)), "MultiSig: Only Wallet can access.")
	...
	t.Run("Claim for Reward, restake", func(t *testing.T) {
		//1. claim, not restake - success cases
		t.Run("Success cases", func(t *testing.T) {
			callOpts := new(bind.CallOpts)
			// 1-1. owner or claimer can call claim function of operator contract right away
			distributeReward(t, g, stateDB, rewardAmount, s1.Staker.Address)
			// restake the reward
			receipt, err = g.ExpectedOk(g.ClaimViaOperatorContract(owner1, s1, true))
			require.NoError(t, err)
			pendingReward := findEvents("UserRewardUpdated", receipt.Logs)[0]["pendingReward"].(*big.Int)
			require.Equal(t, g.balanceAt(t, ctx, govwbft.GovStakingAddress, nil), new(big.Int).Add(new(big.Int).Mul(minStaking, common.Big2), pendingReward))
			...
			// 1-2. owner can multiSig claim function of operator contract
			beforeStakedAmt := govwbft.StakerInfo(stateDB, s1.Staker.Address).TotalStaked

			distributeReward(t, g, stateDB, rewardAmount, s1.Staker.Address)
			callData, _ := operatorAbi.Pack("claim", s1.Staker.Address, true)
			receipt, err = g.ExpectedOk(g.SubmitTransaction(owner1, operatorSampleAddr, common.Big0, callData))
			require.NoError(t, err)
			txId := findEvents("SubmitTransaction", receipt.Logs)[0]["txIndex"].(*big.Int)
			...
			require.NoError(t, g.operatorContract.Call(callOpts, &[]interface{}{&tracedReward}, "rewardAmount"))
			require.True(t, tracedReward.Cmp(common.Big0) == 0)
			// 1-3. owner can submit claim function of governance contract
			beforeStakedAmt = govwbft.StakerInfo(stateDB, s1.Staker.Address).TotalStaked
		...
		t.Run("Claim for Reward, not restake", func(t *testing.T) {
			t.Run("Success cases", func(t *testing.T) {
				callOpts := new(bind.CallOpts)
				var beforeTracedReward *big.Int
				require.NoError(t, g.operatorContract.Call(callOpts, &[]interface{}{&beforeTracedReward}, "rewardAmount"))
				// 1-1. owner or claimer can call claim function of operator contract right away
				distributeReward(t, g, stateDB, rewardAmount, s1.Staker.Address)
				// restake the reward
				receipt, err = g.ExpectedOk(g.ClaimViaOperatorContract(owner1, s1, false))
				require.NoError(t, err)
				pendingReward := findEvents("UserRewardUpdated", receipt.Logs)[0]["pendingReward"].(*big.Int)
				var tracedReward *big.Int
				require.NoError(t, g.operatorContract.Call(callOpts, &[]interface{}{&tracedReward}, "rewardAmount"))
				...
				// 1-2. owner can multiSig claim function of operator contract
				distributeReward(t, g, stateDB, rewardAmount, s1.Staker.Address)
				callData, _ := operatorAbi.Pack("claim", s1.Staker.Address, false)
				receipt, err = g.ExpectedOk(g.SubmitTransaction(owner1, operatorSampleAddr, common.Big0, callData))
				require.NoError(t, err)
				txId := findEvents("SubmitTransaction", receipt.Logs)[0]["txIndex"].(*big.Int)
		...
		t.Run("Withdraw reward", func(t *testing.T) {
			...
			t.Run("Failure case", func(t *testing.T) {
				// 2-1. cannot call function for single owner
				ExpectedRevert(t, g.ExpectedFail(g.WithdrawRewardAmount(owner1, owner1.From, claimedReward)), "MultiSig: Only Wallet can access.")
			})
			t.Run("Success case", func(t *testing.T) {
				beforeBalance := g.balanceAt(t, ctx, owner4.From, nil)
				// withdraw claimedReward to owner4
				callData, _ := operatorAbi.Pack("withdrawRewardAmount", owner4.From, claimedReward)
				receipt, err = g.ExpectedOk(g.SubmitTransaction(owner1, operatorSampleAddr, common.Big0, callData))
				require.NoError(t, err)
				txId := findEvents("SubmitTransaction", receipt.Logs)[0]["txIndex"].(*big.Int)
		...
		t.Run("Withdraw fee", func(t *testing.T) {
			...
			t.Run("Failure case", func(t *testing.T) {
				...
				ExpectedRevert(t, g.ExpectedFail(g.WithdrawFeeAmount(owner1, owner1.From, claimedFee)), "MultiSig: Only Wallet can access.")
			})
			//2. withdraw fee - success case
			t.Run("Success case", func(t *testing.T) {
				beforeBalance := g.balanceAt(t, ctx, owner4.From, nil)
				// withdraw claimedFee to owner4
				callData, _ := operatorAbi.Pack("withdrawFeeAmount", owner4.From, claimedFee)
				receipt, err = g.ExpectedOk(g.SubmitTransaction(owner1, operatorSampleAddr, common.Big0, callData))
				require.NoError(t, err)
				txId := findEvents("SubmitTransaction", receipt.Logs)[0]["txIndex"].(*big.Int)
		...
		//var firstUnbonding *big.Int
		//var secondUnbondig *big.Int
		...
		t.Run("Unstake and withdraw unstaked amount", func(t *testing.T) {
			...
			t.Run("Unstake failure case", func(t *testing.T) {
				// 1-1. cannot call function for single owner
				ExpectedRevert(t, g.ExpectedFail(g.SingleOwnerUnstake(owner1, stakedAmt)), "MultiSig: Only Wallet can access.")
			})
		...
		t.Run("Withdraw unstaked amount from govContract", func(t *testing.T) {
			...
			t.Run("Failure case", func(t *testing.T) {
				...
				ExpectedRevert(t, g.ExpectedFail(g.WithdrawViaOperatorContract(owner1, common.Big1)), "MultiSig: Only Wallet can access.")
			})
		...
		t.Run("Withdraw unstaked amount from operator contarct", func(t *testing.T) {
			...
			t.Run("Failure case", func(t *testing.T) {
				// 1-1. cannot call function for single owner
				ExpectedRevert(t, g.ExpectedFail(g.WithdrawUnstakedAmount(owner1, owner1.From, stakedAmt)), "MultiSig: Only Wallet can access.")
				...
				ExpectedRevert(t, g.ExpectedFail(g.SubmitTransaction(owner1, operatorSampleAddr, stakedAmt, callData)), "Operator: Use proper withdraw functions to transfer value from contract")
				require.NoError(t, err)
			})
			//2. withdraw fee - success case
			t.Run("Success case", func(t *testing.T) {
				...
				beforeBalance := g.balanceAt(t, ctx, owner4.From, nil)
				// withdraw claimedFee to owner4
				callData, _ := operatorAbi.Pack("withdrawUnstakedAmount", owner4.From, stakedAmt)
				receipt, err = g.ExpectedOk(g.SubmitTransaction(owner1, operatorSampleAddr, common.Big0, callData))
				require.NoError(t, err)
				txId := findEvents("SubmitTransaction", receipt.Logs)[0]["txIndex"].(*big.Int)
